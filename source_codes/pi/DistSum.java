...
public final class DistSum extends Configured implements Tool {
  ...
  public static abstract class Machine {
    ...
    static void compute(Summation sigma,
      ...
      sigma.compute();  // compute
      final long duration = System.currentTimeMillis() - start; // time spended
      final TaskResult result = new TaskResult(sigma, duration); // result (compute result, time)

      LOG.info(s = "result=" + result); // log
      context.setStatus(s);
